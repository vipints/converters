...
#
# Written (W) 2010-2012 Vipin T Sreedharan, Friedrich Miescher Laboratory of the Max Planck Society
...
USAGE: gff3_to_gtf_converter.pl in.gff3 out.gtf
);
if (scalar(@ARGV) != 2) {
    print $usage;
    exit
...
while (my $feature = $inGFF->next_feature() ) {
    if ($feature->type->name eq 'mRNA' || $feature->type->name eq 'miRNA' || $feature->type->name eq 'ncRNA' || $feature->type->name eq 'rRNA' || $feature->type->name eq 'snoRNA' || $feature->type->name eq 'snRNA' || $feature->type->name eq 'tRNA' || $feature->type->name eq 'misc_RNA' || $feature->type->name eq 'processed_transcript' || $feature->type->name eq 'transcript' || $feature->type->name eq 'scRNA' || $feature->type->name eq 'pseudogenic_transcript') {
