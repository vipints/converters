...
#
# Description : Convert a BED format file to GFF3.
...
def __main__():
    ...
    try:
        ...
        sys.stderr.write('\nBED format file fail to open, Cannot continue...\n\tUSAGE: bed_to_gff3_converter.py <bed format file> > <gff3 file>\n\n')
        sys.exit(-1)
    print '##gff-version 3'
    for line in bed_fh: 
        line = line.strip( '\n\r' ).split( '\t' )
        if re.match('#', line[0]):continue
        assert (len(line) == 12), '\t'.join(line) # considering only BED lines with 12 fields
        ...
        print line[0] + '\tfml_bed2gff\tgene\t' + str(int(line[1]) + 1) + '\t' + line[2] + '\t' + line[4] + '\t' + line[5] + '\t.\t' + 'ID=Gene:' + line[3] + ';Name=Gene:' + line[3] 
        print line[0] + '\tfml_bed2gff\ttranscript\t' + str(int(line[1]) + 1) + '\t' + line[2] + '\t' + line[4] + '\t' + line[5] + '\t.\t' + 'ID=' + line[3] + ';Name=' + line[3] + ';Parent=Gene:' + line[3]
        st = int(line[1])
        for ex_cnt in range(int(line[-3])):
            start = st + int(rstart[ex_cnt]) + 1
            stop = start + int(exon_len[ex_cnt]) - 1
            print line[0] + '\tfml_bed2gff\texon\t' + str(start) + '\t' + str(stop) + '\t' + line[4] + '\t' + line[5] + '\t.\t' + 'Parent=' + line[3]
    bed_fh.close()
